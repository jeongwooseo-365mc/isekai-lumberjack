#!/usr/bin/env python3
"""Validate runtime assets, mobile layout rules, and packaging inputs for v1.2.5."""
...
def main() -> int:
    backgrounds = [
        ...
        "story_intro2.png",
        "story_intro3.png",
        *[f"forest{i}.png" for i in range(1, 5)],
        *[f"mine{i}.png" for i in range(1, 5)],
        *[f"dungeon{i}.png" for i in range(1, 5)],
    ...
    check_image("assets/items/dark_blessing.png", (256, 256))
    with Image.open(ROOT / "assets/items/dark_blessing.png") as blessing:
        check("A" in blessing.getbands() and blessing.getchannel("A").getextrema()[0] == 0, "dark blessing icon must retain transparency")
    ...
    resources += [f"tome_{kind}.png" for kind in ("wood", "ore", "gold")]
    ...
    check_image("assets/ui/intro_title_logo.png")
    check_image("assets/ui/studio_logo.png")
    ...
    check('const APP_VERSION = "1.2.5"' in game_js and 'const SAVE_VERSION = "1.1.0"' in game_js and 'const SAVE_KEY = "isekai_lumberjack_save_v11"' in game_js, "v1.2.5 must preserve the v1.1 save schema")
    ...
    check("{wood2:2000,gold2:2000,tome_wood:100}" in game_js and "{wood2:800,ore2:800,gold2:400,tome_wood:40,tome_ore:40,tome_gold:20}" in game_js, "divine recipes retain existing resources and add secret tomes")
    check('data-type="secret">비밀교환소' in game_js and "SECRET_EXCHANGE_OFFER_COUNT = 4" in game_js, "workshop must expose four-item secret exchange windows")
    check("date.getHours()/3" in game_js and "date.setHours((Math.floor(date.getHours()/3)+1)*3,0,0,0)" in game_js, "secret exchange must use local 00/03/06/09/12/15/18/21 fixed boundaries")
    check("template=>level>=template.minLevel" in game_js and "minLevel:60" in game_js, "upper-tier secret offers must stay locked until level 60")
    ...
    check("v1.2.5-android-arm64.apk" in release_workflow and "v1.2.5-windows-[bundle]" in release_workflow, "release artifact names must use v1.2.5")
    ...
    check("screen_height * 61 / 100" in android_smoke, "Android smoke test must tap the v1.2.5 start button below the centered logo")
    ...
    check(tauri_config.get("version") == "1.2.5", "Tauri version must be 1.2.5")
